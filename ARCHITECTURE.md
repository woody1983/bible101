...
- [ ] 4. **开发记录 Comment 已添加到 Issue** ⭐ 最容易遗忘！
- [ ] 5. 推送到 GitHub（git push）
- [ ] 6. 创建 PR（如需要）
```

**🔴 特别注意第 4 项：开发记录 Comment**
...
### 0.12 PR 提交准则：关闭 Issue 并记录过程

**⚠️ 重要：提交 PR 时必须一并关闭相关 Issue，并将结果和过程作为 Comment**

#### PR 提交流程
```
开发完成 → 写 Comment → 推送代码 → 创建 PR → 关联 Issue → 关闭 Issue
         ↑
    最容易遗忘！务必先做！
```
...
### 0.13 TDD 拆解和 Sub-tasks 创建规则

**触发条件：** 用户在 Issue 中要求 "TDD 拆解" 或使用关键字 `#tdd-breakdown#`
...
分析 Issue 需求和复杂度
    ↓
在 Issue Comment 中创建 Sub-tasks 列表
    ↓
等待用户确认
    ↓
用户确认后开始实施
...
基于 Issue 需求分析，拆解为以下子任务：

### Sub-task 1: [功能/模块名称]
**目标**: [具体要实现什么]

**TDD 步骤**:
- [ ] 1. 编写测试: [测试描述，包括输入和预期输出]
- [ ] 2. 运行测试: 确认测试失败（红色）
- [ ] 3. 实现功能: [最小实现代码]
- [ ] 4. 运行测试: 确认测试通过（绿色）
- [ ] 5. 重构代码: [优化代码结构]
...
1. **单一职责**: 每个 sub-task 只负责一个功能点
2. **可测试**: 每个 sub-task 必须有明确的测试标准
3. **独立性**: 尽量减少 sub-task 之间的依赖
4. **时间盒**: 每个 sub-task 预估时间不超过 4 小时

#### 复杂度评估

根据 Issue 复杂度决定 sub-task 数量：

| 复杂度 | Sub-tasks 数量 | 示例 |
|--------|---------------|------|
| 简单 | 1-2 个 | Bug 修复、小功能添加 |
| 中等 | 3-5 个 | 新组件开发、API 集成 |
| 复杂 | 6+ 个 | 大功能模块、架构重构 |
